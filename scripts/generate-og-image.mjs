...
let interRegular = null; let interBold = null;
...
if ((!interRegular || !interBold) && typeof fetch === 'function') {
    const fontFiles = [
        ['Inter-Regular.ttf', 'https://rsms.me/inter/font-files/Inter-Regular.ttf'],
        ['Inter-Bold.ttf', 'https://rsms.me/inter/font-files/Inter-Bold.ttf']
    ];
    for (const [name, url] of fontFiles) {
        try {
            if ((name.includes('Regular') && interRegular) || (name.includes('Bold') && interBold)) continue;
            ...
            if (res.ok) {
                ...
                try {
                    ...
                    console.log('Downloaded', name, 'for OG generation.');
                } catch {}
            }
        } catch {}
    }
...
async function buildSvg() {
    ...
    if (satoriAvailable && satori && (interRegular || interBold)) {
        try {
            ...
            const svgResult = await satori({
                ...
                props: {
                    ...
                    style: { display: 'flex', width: '100%', height: '100%', backgroundColor: '#0b1220', fontFamily: 'Inter, system-ui, sans-serif', position: 'relative', color: '#ffffff', padding: 0, margin: 0 },
                    children: [
                        ...
                        {
                            type: 'div', props: {
                                style: { display: 'flex', flexDirection: 'column', padding: 70, maxWidth: 950, position: 'relative', zIndex: 2 }, children: [
                                    {
                                        type: 'div', props: {
                ...
                width, height, fonts: [
                    interRegular ? { name: 'Inter', data: interRegular, weight: 400, style: 'normal' } : null,
                    interBold ? { name: 'Inter', data: interBold, weight: 700, style: 'normal' } : null
                ].filter(Boolean)
            });
            return svgResult;
        } catch (e) {
        ...
        console.log('Skipping Satori OG generation: no Inter TTF fonts present (expecting public/fonts/Inter-Regular.ttf or Inter-Bold.ttf).');
    }
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">\n  <defs>\n    <linearGradient id="${gradientId}" x1="0%" y1="0%" x2="100%" y2="100%">\n      <stop offset="0%" stop-color="#0ea5e9" />\n      <stop offset="50%" stop-color="#a855f7" />\n      <stop offset="100%" stop-color="#f472b6" />\n    </linearGradient>\n    <filter id="glow" x="-20%" y="-20%" width="140%" height="140%">\n      <feGaussianBlur stdDeviation="12" result="blur" />\n      <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>\n    </filter>\n  </defs>\n  <rect width="100%" height="100%" fill="#0b1220"/>\n  <g opacity="0.85" transform="translate(885 90) scale(2.1)">${mark}</g>\n  <rect x="40" y="40" width="1120" height="550" rx="48" fill="url(#${gradientId})" fill-opacity="0.08" stroke="rgba(255,255,255,0.08)"/>\n  <text x="100" y="220" font-family="'Inter',system-ui,sans-serif" font-weight="700" font-size="108" fill="#ffffff" filter="url(#glow)">${persona.brand}</text>\n  <text x="102" y="310" font-family="'Inter',system-ui,sans-serif" font-size="44" fill="#86e1ff" font-weight="600">${persona.tagline}</text>\n  <text x="104" y="370" font-family="'Inter',system-ui,sans-serif" font-size="30" fill="#d1d5db" opacity="0.95" style="line-height:1.3;">\n    <tspan x="104" dy="0">${defaultMeta.description}</tspan>\n  </text>\n  <text x="104" y="470" font-family="'Inter',system-ui,sans-serif" font-size="26" fill="#94a3b8" letter-spacing="3">PERFORMANCE • ACCESSIBILITY • PWA</text>\n</svg>`;
}
